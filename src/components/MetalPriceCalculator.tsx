...
export default function MetalPriceCalculator({ initialRates, compact = false }: Props) {
  ...
  return (
    <div className={`bg-white rounded-3xl border-2 border-amber-300 shadow-lg overflow-hidden ${compact ? 'p-6' : 'p-6 sm:p-8'}`}>
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {[
                ...
                >
                  ...
                  <div className="text-[10px] text-slate-500 font-medium mt-0.5">{m.sub}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex rounded-xl bg-slate-100 p-1 text-xs">
                {(['tola', 'grams', 'lal', 'aana'] as const).map((u) => (
                  ...
                  >
                    {u === 'tola' ? 'Tola (तोला)' : u === 'grams' ? 'Grams (ग्राम)' : u === 'lal' ? 'Lal (लाल)' : 'Aana (आना)'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full text-lg font-extrabold text-slate-900 px-4 py-3 rounded-2xl border-2 border-slate-200 focus:outline-hidden focus:border-amber-400 bg-white shadow-2xs"
              />
                ...
                <>
                  {[0.5, 1, 1.5, 2, 3, 5].map((val) => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 text-xs font-bold border border-amber-200 transition-colors"
                    >
                      {val} Tola
                    </button>
                  ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex text-[11px] bg-slate-200 rounded-lg p-0.5 font-bold">
                  <button
                    type="button"
                    onClick={() => setMakingChargeType('percent')}
                    className={`px-2 py-0.5 rounded-md ${makingChargeType === 'percent' ? 'bg-white shadow-2xs text-slate-900' : 'text-slate-600'}`}
                  >
                    %
                  </button>
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-md ${makingChargeType === 'flat' ? 'bg-white shadow-2xs text-slate-900' : 'text-slate-600'}`}
                  >
                    NPR
                  </button>
                </div>
        ...
        <div className="lg:col-span-5 flex flex-col justify-between p-6 sm:p-7 rounded-3xl bg-amber-50/50 border-2 border-amber-300 shadow-md">
          <div>
            <div className="flex items-center justify-between pb-4 border-b border-amber-200">
              <span className="text-xs font-extrabold text-amber-900 uppercase tracking-widest flex items-center gap-1.5">
            ...
            <div className="my-6">
              ...
              <div className="text-3xl sm:text-4xl font-extrabold text-slate-950 font-serif-luxury tracking-tight mt-1">
